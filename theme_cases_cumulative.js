...
const themeCasesCumulative = {
	...
	choroplethColorScale: d3.scaleSequential(function (d) { 
		return d3.interpolateMagma(nthPowScale(0.7)(casesCumulative_logScale(d))
		)}) // change to interpolateGreys, 6000 for illustrations
	,
	...
	cellsAndLabelsFcn: function (feat, date) {
		var getValsFunc = expBase10CellsAndLabelsFunc(100*Math.cbrt(10),casesCumulative_legendMax);
		return getValsFunc();
	},
	...
	tooltipTextFcn: function (feat, date) {
		...
		var case_count = getValue(feat,date,'positive',perMillion = false);
		var case_rate = getValue(feat,date,'positive',perMillion = true); // choroplethValue(feat,date);
		msg = "<p>Population: " + withCommas(pop) + "</p>";
		msg += "<p>" + withCommas(case_count) + " cases</p>";
		msg += "<p>" + rateText(case_rate) + " cases per million</p>";		
		return msg;
	}
